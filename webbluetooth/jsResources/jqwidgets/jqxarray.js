(function ($)
{
    ...
    $.extend($.jqx._jqxArray.prototype, {
        ...
        addDimension: function (changeValueDimensions)
        {
            ...
            indexer.find('.jqx-icon-arrow-up').parent()[0].className += ' jqx-array-indexer-increment';
            indexer.find('.jqx-icon-arrow-down').parent()[0].className += ' jqx-array-indexer-decrement';
            ...
            if (that.type !== 'none')
            {
                ...
                for (var i = 0; i < cells.length; i++)
                {
                    for (var j = 0; j < cells[i].length; j++)
                    {
                        ...
                        cellWidget[0].className += ' jqx-fill-state-disabled jqx-array-element-empty';
            ...
            if (value !== oldvalue)
            {
                ...
                switch (key)
                {
                    ...
                    case 'disabled':
                        if (value)
                        {
                            object.element.className += ' jqx-fill-state-disabled';
                        } else
                        {
                    ...
                    case 'theme':
                        ...
                        object._bigContainer.className += ' jqx-fill-state-pressed-' + value;
                        ...
                        for (var m = 0; m < object.rows; m++)
                        {
                            for (var n = 0; n < object.columns; n++)
                            {
                                ...
                                td[0].className += ' jqx-fill-state-pressed-' + value;
                            }
                        }
        ...
        _addClasses: function ()
        {
            ...
            if (that.disabled)
            {
                that.element.className += ' jqx-fill-state-disabled';
            }
        },
        ...
        {
            var that = this,
                gapClass = that._elementGap ? ' jqx-array-table-data-gap' : '';
            that._cells = [];
            ...
            for (var i = 0; i < that.rows; i++)
            {
                ...
                currentRow.className += ' jqx-array-table-row';
                that._cells.push([]);
                for (var j = 0; j < that.columns; j++)
                {
                    var currentCell = document.createElement('td');
                    currentCell.className += ' ' + that.toThemeProperty('jqx-fill-state-pressed') + ' jqx-array-table-data' + gapClass;
        ...
        _initializeWidget: function (i, j)
        {
            ...
            widget[0].className += ' jqx-array-element jqx-array-element-' + that.element.id;
            if (initialValue === undefined)
            {
                widget[0].className += ' jqx-fill-state-disabled jqx-array-element-empty';
            }
        ...
        _addRemoveColumn: function (action)
        {
            ...
            if (action === 'add')
            {
                var gapClass = that._elementGap ? ' jqx-array-table-data-gap' : '',
                    rows = that._tableBody.children();
                for (var i = 0; i < that._cells.length; i++)
                {
                    var addToRow = that._cells[i],
                        newCell = document.createElement('td');
                    newCell.className += ' ' + that.toThemeProperty('jqx-fill-state-pressed') + ' jqx-array-table-data' + gapClass;
        ...
        _addRemoveRow: function (action)
        {
            ...
            if (action === 'add' && (that.dimensions > 1 || (that.dimensions === 1 && that.columns === 1)))
            {
                ...
                var newRow = document.createElement('tr'),
                    ...
                    newCells = [];
                newRow.className += ' jqx-array-table-row';
                for (var j = 0; j < that.columns; j++)
                {
                    var currentNewCell = document.createElement('td');
                    currentNewCell.className += ' ' + that.toThemeProperty('jqx-fill-state-pressed') + ' jqx-array-table-data' + gapClass;
        ...
        _updateWidgetWidth: function (propertyChangedHandler)
        {
            ...
            document.getElementById(that.element.id + 'CentralContainer').style.width = centralContainerWidth + 'px';
        ...
        _addSelectionClass: function (x, y, td, skipSelectionCheck)
        {
            ...
            if (that.showSelection && that._absoluteSelectionStart !== undefined && that._absoluteSelectionEnd !== undefined)
            {
                ...
                {
                    td.className += ' jqx-array-element-selected';
                } else
                {
            ...
            that.addHandler(vScrollbar, 'valueChanged.jqxArray' + id, function (event)
            {
                if (that.type !== 'none')
                {
                    if (that._suppressScrollbarEvent !== true)
                    {
                        ...
                        that._moveScrollbar(vScrollbar, 'vertical', index, event.currentValue);
                    } else
                    {
        ...
        _addHScrollBarHandlers: function ()
        {
            ...
            that.addHandler(hScrollbar, 'valueChanged.jqxArray' + id, function (event)
            {
                if (that.type !== 'none')
                {
                    if (that._suppressScrollbarEvent !== true)
                    {
                        that._moveScrollbar(hScrollbar, 'horizontal', 0, event.currentValue);
                    } else
                    {
                        that._suppressScrollbarEvent = false;
                    }
