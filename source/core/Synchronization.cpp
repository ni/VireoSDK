...
static void CleanUpQueueRefNum(intptr_t arg) {
    ...
    QueueRef queueRef = NULL;
    if (QueueRefNumManager::QueueRefManager().DisposeAlias(refnum)) {
        // was a named clone
            ...
            if (!refnumVal) {
                ...
                if (!refnumVal)
                    ...
                    if (createdPtr)
                        *createdPtr = true;
                    if (name) {
                        QueueRefNumManager::QueueRefManager().NamedRefNumMap()[name] = refnumVal;
...
DEFINE_VIREO_BEGIN(Synchronization)
    ...
    DEFINE_VIREO_FUNCTION_CUSTOM(ObtainQueue, QueueRef_Obtain, "p(o(QueueRefNum queue)i(Int32 maxsize)i(String name)i(Boolean create)o(Boolean created)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(ReleaseQueue, QueueRef_Release, "p(i(QueueRefNum queue)o(String name)o(Array remainingElems)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(Enqueue, QueueRef_Enqueue, "p(io(QueueRefNum queue)i(* element)i(Int32 timeOut)o(Boolean timedOut)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(EnqueueFront, QueueRef_EnqueueFront, "p(io(QueueRefNum queue)i(* element)i(Int32 timeOut)o(Boolean timedOut)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(LossyEnqueue, QueueRef_LossyEnqueue, "p(io(QueueRefNum queue)i(* element)o(* overflowElem)o(Boolean overflowed)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(Dequeue, QueueRef_Dequeue, "p(io(QueueRefNum queue)o(* element)i(Int32 timeOut)o(Boolean timedOut)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(PeekQueue, QueueRef_PeekQueue, "p(io(QueueRefNum queue)o(* element)i(Int32 timeOut)o(Boolean timedOut)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(FlushQueue, QueueRef_FlushQueue, "p(i(QueueRefNum queue) o(Array remainingElems)io(" ERROR_CLUST_TYPE_STRING " err))")
    DEFINE_VIREO_FUNCTION_CUSTOM(GetQueueStatus, QueueRef_GetQueueStatus, "p(i(QueueRefNum queue)i(Boolean returnElems)o(Int32 maxSize)o(String name)o(Int32 pendingRemove)o(Int32 pendingInsert)o(Int32 numElems)o(Array elements)io(" ERROR_CLUST_TYPE_STRING " err))")
